      ...
      <AnimatePresence exitBeforeEnter>
        {capturedUri ? (
          ...
          >
            ...
            <XStack
              backgroundColor={isLight ? "rgba(255, 255, 255, 0.9)" : "rgba(30, 30, 35, 0.85)"}
              padding="$3"
              borderRadius={100}
              gap="$3"
              alignItems="center"
            ...
            >
              {isDetecting ? (
                <XStack flex={1} alignItems="center" justifyContent="center" gap="$3" height={55}>
                  <MotiView
                  ...
                  <Text fontSize={14} fontWeight="900" color={colors.text} letterSpacing={2}>
                    IDENTIFYING...
                  </Text>
                </XStack>
              ) : (
                <>
                  ...
                  <Button
                    ...
                    width={55}
                    circular
                    backgroundColor={isLight ? "$gray4" : "rgba(255,255,255,0.05)"}
                    borderWidth={1}
                    borderColor={isLight ? "$gray6" : "rgba(255,255,255,0.1)"}
                    icon={<RefreshCcw size={20} color={colors.text} opacity={0.8} />}
                  ...
                  <Button
                    ...
                    height={55}
                    borderRadius={100}
                    backgroundColor={reachedLimit ? "$orange10" : colors.primary}
                    onPress={() => reachedLimit ? onUpsellTrigger?.() : onDetect()}
          ...
          >
            <YStack alignItems="center" gap="$4">
              ...
              <View width={90} height={90} alignItems="center" justifyContent="center">
                <MotiView
                  from={{ scale: 1, opacity: 0.4 }}
                  animate={{ scale: 1.5, opacity: 0 }}
                  transition={{ loop: true, duration: 2000, type: 'timing' }}
                  style={{ position: 'absolute', width: 75, height: 75, borderRadius: 40, borderWidth: 1, borderColor: 'white' }}
                />
                <Button
                  onPress={() => reachedLimit ? onUpsellTrigger?.() : onCapture()}
                  width={75}
                  height={75}
                  borderRadius={40}
                  ...
                  padding={0}
                >
                  <Circle size={64} borderWidth={1.5} borderColor="rgba(0,0,0,0.05)">
